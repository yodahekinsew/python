...
class Image:
    ...
    def apply_kernel(self,kernel):
        #kernel is a list of integers to apply to manipulate image
        #function applies kernel to image
        #creates new image and saves the new pixel values to that new image
        ...
        for y in range(result.height):
            for x in range(result.width):
                ...
                half = int((len(kernel)**.5)//2)
                
                for j in range(-half, half+1):
                    for i in range(-half, half+1):
                        new_color += ((self.get_pixel(x + i,y + j))*(kernel[kernel_index]))
                        kernel_index+=1
