...
class Image:
    ...
    def get_pixel(self, pixel_number):
        #Pixel_number is an integer
        #Function is used to index through the list of pixels using pixel number
        return self.pixels[pixel_number]
    
    def set_pixel(self, pixel_number, c):
        #Pixel_number is an integer
        #c is an integer assigned to a pixel value (between 0 and 255)
        #Function is used to place pixel value in Image
        self.pixels[pixel_number] = c

    def apply_per_pixel(self, func):
        #Func is a certain lambda function that is applied to pixel values to invert them
        #This function applies that func to all the pixels in the image
        #Stores new values of pixels in a new image and returns that new image
        result = Image.new(self.width, self.height)
        pixel_number = 0
        for x in range(result.width):
            for y in range(result.height):
                color = self.get_pixel(pixel_number)
                newcolor = func(color)
                result.set_pixel(pixel_number, newcolor)
                pixel_number += 1
        print(pixel_number)
        return result

    def inverted(self):
        #Call for rest of function
